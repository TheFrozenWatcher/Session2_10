...
public class TennisGameTest {
    ...
    public void checkAllScores() {
        int maxScore = Math.max(this.player1Score, this.player2Score);
        int actualPlayer1Score = 0;
        int actualPlayer2Score = 0;
        for (int i = 0; i < maxScore; i++) {
            if (i < this.player1Score)
                actualPlayer1Score += 1;
            if (i < this.player2Score)
                actualPlayer2Score += 1;
        }
        assertEquals(this.expectedScore, TennisGame.getScore(actualPlayer1Score, actualPlayer2Score));
    }
